...
internal static class PitchVelocitySolver
{
    ...
    {
        Debug.Log("[ç≈ìKâª] äJén");

        Vector3 currentVelocity = EstimateInitialVelocityImproved(
            startPoint, targetPoint, spinAxisNormalized,
            spinRateRPM, liftCoefficient, desiredSpeed);
        ...
        for (int i = 0; i < BallPhysicsConstants.MAX_OPTIMIZATION_ITERATIONS; i++)
        {
            var config = new BallPhysicsCalculator.SimulationConfig
            {
                ...
                StopAtZ = settings?.StopAtTarget == true
                    ? settings.StopPosition.z : (float?)null,
                BounceSettings = bounceSettings
            };

            List<Vector3> testTrajectory = BallTrajectorySimulator.SimulateTrajectory(
                startPoint, currentVelocity, spinAxisNormalized,
                spinRateRPM, liftCoefficient, config);

            if (testTrajectory.Count == 0)
            {
                Debug.LogWarning("[ç≈ìKâª] ãOìπåvéZé∏îs");
                break;
            }
            ...
            {
                float zAdj = error.z * Mathf.Lerp(
                    BallPhysicsConstants.Z_ADJUSTMENT_INITIAL,
                    BallPhysicsConstants.Z_ADJUSTMENT_FINAL, progress);
                currentVelocity.z += zAdj;
            }

            Vector3 xyAdj = new Vector3(error.x, error.y, 0f) *
                Mathf.Lerp(BallPhysicsConstants.XY_ADJUSTMENT_INITIAL,
                           BallPhysicsConstants.XY_ADJUSTMENT_FINAL, progress);
            currentVelocity += xyAdj;
            ...
            if (Mathf.Abs(speedError) > desiredSpeed * BallPhysicsConstants.SPEED_ERROR_THRESHOLD)
            {
                float factor = Mathf.Lerp(
                    BallPhysicsConstants.SPEED_ADJUSTMENT_INITIAL,
                    BallPhysicsConstants.SPEED_ADJUSTMENT_FINAL, progress);
                currentVelocity = currentVelocity.normalized *
                    Mathf.Lerp(currentSpeed, desiredSpeed, factor);
            }
        }
        ...
        float desiredSpeed)
    {
        ...
        float dragFactor = BallPhysicsConstants.DRAG_FACTOR_BASE +
            (BallPhysicsConstants.DRAG_COEFFICIENT * BallPhysicsConstants.AIR_DENSITY *
             BallPhysicsConstants.CROSS_SECTION * desiredSpeed) /
        ...
        float magnusAccel = BallPhysicsConstants.MAGNUS_FORCE_HALF *
            BallPhysicsConstants.AIR_DENSITY * desiredSpeed * desiredSpeed *
            BallPhysicsConstants.CROSS_SECTION * liftCoefficient /
            BallPhysicsConstants.BALL_MASS;
        float magnusDisplacement = BallPhysicsConstants.GRAVITY_HALF *
            magnusAccel * estimatedTime * estimatedTime;

        float zSpeed = displacement.z / estimatedTime;
        float xSpeed = displacement.x / estimatedTime;
        float verticalSpeed = verticalDist / estimatedTime +
            gravity * estimatedTime * BallPhysicsConstants.GRAVITY_HALF;

        float magnusVerticalEffect = magnusDir.y * magnusDisplacement / estimatedTime;
        verticalSpeed -= magnusVerticalEffect *
            BallPhysicsConstants.MAGNUS_VERTICAL_CORRECTION_FACTOR;

        return new Vector3(xSpeed, verticalSpeed, zSpeed);
    }
}